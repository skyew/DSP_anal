...
```{r data_get, results='hide', include = F, message = F, warning = F, echo = F}
...
```{r data_prep, fig.width=6, fig.height=6}
...
```{r plfa_graphs,fig.width=6, fig.height=6}
...
```{r lab_graphs,fig.width=6, fig.height=6}
...
lab_state_pullman <- labs %>% filter(Property %in% c("Water Stable Aggregates", "ß-Glucosidase", "POX_C", "Total Carbon" ) ) %>%
  ...
  strip.position = "top", page = 1) +
  theme_economist() +
  labs(y = '', x = '\n', fill = '',title = 'Pullman: 0 - 5 cm surface Samples')
